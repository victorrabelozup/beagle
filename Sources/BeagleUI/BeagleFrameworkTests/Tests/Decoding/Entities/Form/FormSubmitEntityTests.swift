...
final class FormSubmitEntityTests: XCTestCase {
    ...
    func test_whenDecodingAValidJson_itShouldReturnAValidObject() {
        ...
        // When
        let object = try? WidgetDecoder().decodeToWidget(ofType: FormSubmit.self, from: jsonData)
    ...
    func test_whenDecodingAnInvalidChild_itShouldThrowDecodingError() {
        ...
        XCTAssertThrowsError(
            _ = try FormSubmitEntity(childContainer: childContainer),
    ...
    func test_whenMapToWidgetIsCalled_thenItShouldReturnAFormEntity() {
        ...
        guard let sut = try? FormSubmitEntity(
