...
final class LoadingViewTests: XCTestCase {
    ...
    func test_awakeFromNib_shouldCallSetup() {
        ...
        let containsBlurView = sut.subviews.contains {
            return $0.alpha == 0.25 && $0.backgroundColor == .black
        }
        let containsActivityIndicator = sut.subviews.contains(where: { $0 is UIActivityIndicatorView } )
    ...
    func test_startAnimating() {
        // Given
        let sut = LoadingView()
        let activityIndicator = sut.subviews.first(where: { $0 is UIActivityIndicatorView } ) as? UIActivityIndicatorView
